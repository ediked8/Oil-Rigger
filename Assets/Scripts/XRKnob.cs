...
namespace Unity.VRTemplate
{
    ...
    /// (Changed to rotate around the X axis / use YZ plane for input)
    /// </summary>
    public class XRKnob : XRBaseInteractable
    {
        ...
        struct TrackedRotation
        {
            ...
            /// </summary>
            /// <param name="direction">The YZ vector used to calculate a rotation angle (for X-axis rotation)</param>
            public void SetBaseFromVector(Vector3 direction)
            {
                // Update any accumulated angle
                m_AccumulatedAngle += m_CurrentOffset;

                // Now set a new base angle (use YZ plane for X-axis rotation)
            ...
            {
                // Set the target angle (use YZ plane)
                var targetAngle = Mathf.Atan2(direction.y, direction.z) * Mathf.Rad2Deg;
        ...
        void UpdateRotation(bool freshCheck = false)
        {
            ...
            // We store any data used for determining which rotation to use, then flatten the vectors to the local YZ plane (for X-axis rotation)
            ...
            var localX = Math.Abs(localForward.x);
            // flatten to YZ plane
            localForward.x = 0.0f;
            ...
            // If it's not a fresh check, then we weight the local X up or down to keep it from flickering back and forth at boundaries
            if (!freshCheck)
            {
                if (!m_UpVectorDriven)
                    localX *= (1.0f - (k_ModeSwitchDeadZone * 0.5f));
                else
                    localX *= (1.0f + (k_ModeSwitchDeadZone * 0.5f));
            }
        ...
        void SetKnobRotation(float angle)
        {
            ...
            if (m_Handle != null)
                // rotate around X axis instead of Y
        ...
        void OnDrawGizmosSelected()
        {
            ...
            // For X-axis rotation draw in YZ plane (up and forward)
            var circleX = knobTransform.up;
